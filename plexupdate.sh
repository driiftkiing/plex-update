...
if [ "${CHECKUPDATE}" = "yes" -a "${AUTOUPDATE}" = "no" ]; then
  ...
  local GIT_BRANCHNAME_CURRENT

  GIT_OWNER_CURRENT=$(githubOwnerName)
  GIT_REPONAME_CURRENT=$(githubRepoName)
  GIT_BRANCHNAME_CURRENT=$(gitCurrentBranch)

  # URL for new version check
  UPSTREAM_GIT_URL="${GITHUB_USER_CONTENT_URL}/${GIT_OWNER_CURRENT:-${GIT_OWNER}}/${GIT_REPONAME_CURRENT-${GIT_REPONAME}}/${GIT_BRANCHNAME_CURRENT:-${BRANCHNAME}}"
